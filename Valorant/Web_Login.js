...
router.get('/:credentials', async (req, res) => {
    try {
        ...
        const result = await executeProcedure(username, pass);
...
async function executeProcedure(username, pass) {
    const request = new sql.Request();
    request.input('username', sql.NVarChar, username);
    request.input('pass', sql.NVarChar, pass);
    const result = await request.execute('Web_Login');

    // Prosedürün dönü? de?erini kontrol etme
    if (result.recordset.length > 0) {
        const KOD = result.recordset[0].KOD;
        return { KOD }; // KOD, Token ve TokenID de?erlerini döndür
    } else {
        return null;
    }
}
